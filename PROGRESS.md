...
### 2026-07-26 モックネイル画像を3D風(立体的)に強化
- 背景: 「3Dでリアルにして」という要望を受け、`image_service.py`が生成する
  SVGをより立体的な見た目にした。
- 変更: 各爪に(1)`feDropShadow`によるドロップシャドウ、(2)`clip-path`で
  爪の輪郭内に収めた斜めのハイライト/シャドウの`linearGradient`(円柱状の
  質感)、(3)ぼかしをかけた小さな光沢ハイライト(`<circle>`+`feGaussianBlur`)
  を追加。背景も単色から中心が明るい`radialGradient`にして奥行きを出した。
- 判断と理由:
  1. 光沢ハイライトは既存の"simple"スタイル用ハイライトと衝突しないよう
     `<ellipse>`ではなく`<circle>`にした(既存テスト
     `test_shape_changes_output`が「square形状に`<ellipse>`が含まれない」
     ことを検証しているため、形状差分の判定に影響しないようにした)。
  2. ドロップシャドウは`<g filter="url(#nail-shadow)">`で爪の中身全体を
     1つのグループとして囲み、輪郭全体に対して1つの影が落ちるようにした
     (フレンチ/ニュアンス等、爪1本が複数要素で構成されるスタイルでも
     二重影にならないようにするため)。
  3. `render_nail_image()`のシグネチャ・決定性(同一seedなら同一SVG)は
     変更していないため、既存のtest_image_service.py(6件)はすべて
     無修正のまま成功。カバレッジもimage_service.py含めアプリ全体100%を維持。
